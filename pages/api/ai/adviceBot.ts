...
async function handlePost(req: ExtendedNextApiRequest, res: NextApiResponse) {
    ...
    if (token) {
        verifyJwt(token, (err, decoded) => {
            ...
            if (botId !== oogwayVars.advicebot_id)
                return res
                    .status(401)
    ...
    try {
        ...
        const adviceBotProfile = await getDoc(
            doc(db, 'profiles', oogwayVars.advicebot_id)
        )
        ...
        const adviceBotComment: AIBotComment = {
            ...
            authorUid: oogwayVars.advicebot_id,
            likes: {},
            dislikes: {},
        ...
        const engagemment: FirebaseEngagement = {
            engagerId: oogwayVars.advicebot_id,
            engageeId: post.uid,
            action: 'comment',
            targetId: docRef.id,
            targetObject: 'Post',
